...
import Walleditdetils from "./Walleditdetils";
...
const Walledit = () => {
    ...
    const hendelDelete = id => {
        ...
        swal({
            ...
            .then(willDelete => {
                if (willDelete) {
                    fetch(`https://test.zrsflooring.com/wall/${id}`, {
                        method: "DELETE"
                    })
                        .then(res => res.json())
                        .then(data => {
    ...
    return (
        <div>
            <div className="w-[95%] xl:w-[90%] lg:w-[90%] xl:mx-auto lg:mx-auto md:mx-auto mx-[2%] xl:gap-8 lg:gap-6 md:gap-10 gap-2 grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1 xl:grid-cols-1 my-12 ">
                {
                    datas.map(data => <Walleditdetils key={data._id} data={data} hendelDelete={hendelDelete} ></Walleditdetils>)
                }
            </div>
