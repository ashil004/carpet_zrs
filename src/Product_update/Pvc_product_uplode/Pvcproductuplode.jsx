...
const Pvcproductuplode = () => {
    const pvc = async event => {
        ...
        console.log(vip);
        fetch('https://test.zrsflooring.com/pvc', {
            method: 'POST',
            headers: {
    ...
    return (
        <div className="xl:ml-[32%] xl:mt-[4%] lg:ml-[22%] lg:mt-[4%] md:mt-6 md:ml-[13%] ml-[8%] mt-4 ">
            <h3 className="text-center text-2xl font-semibold text-lime-500">Pvc Product</h3>
            <div className="flex justify-between items-center ">
            <div className="dropdown">
                        <div tabIndex={0}  className="btn m-1"><button className=" text-sm md:text-lg ">Pvc Sub <span className="hidden md:inline">Product </span> </button></div>
                        <ul tabIndex={0} className="dropdown-content z-[1] menu p-2 shadow bg-white rounded-box w-52">
                            <li><Link className="text-sm md:text-lg hover:text-lime-400" to="/admin/wet-met-uplode">Wet Mat  </Link></li>
                            <li><Link className="text-sm md:text-lg hover:text-lime-400" to="/admin/comlimat-met-uplode">SPVC/Coil Mat</Link></li>
                            <li><Link className="text-sm md:text-lg hover:text-lime-400" to="/admin/wet-met-uplode">PVC Mat</Link></li>
                            <li><Link className="text-sm md:text-lg hover:text-lime-400" to="/admin/clear-pvc-mat">Clear PVC</Link></li>
                            
                        </ul>
                    </div>
                <Link to='/admin/pvc-edit'><button className="btn  text-sm md:text-lg ">Edit Product</button></Link>
            </div>
            <form onSubmit={pvc} >
...
export default Pvcproductuplode;