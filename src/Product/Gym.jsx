...
import Gymdetils from "./Gymdetils";
...
const Gym = () => {
    const [agyms, setAgyms] = useState([]);
    useEffect(() => {
        fetch('https://test.zrsflooring.com/gym')
            .then(res => res.json())
            .then(data => setAgyms(data));

    }, [])
    return (
        <div>
            <div className="-mt-24">
                <div className="h-[60vh] mt-24 bg-cover " style={{ backgroundImage: 'url(https://i.ibb.co/bd2Zhnn/Gym-Flooring.jpg)', }}>
                    <div className="hero-overlay "></div>
                    <div className="  text-white -mt-56 ml-12 ">
                        <h1 className="text-3xl font-bold" >Gym Flooring</h1>
                        <p>We Are The BestGym Flooring suppliers in UAE</p>
                    </div>
                </div>
            ...
            <div className="flex  mt-10  space-x-6 m-6 ">
                ...
                <div className="w-[95%] xl:w-[90%] lg:w-[90%] mx-auto xl:gap-8 lg:gap-6 md:gap-10 gap-2 grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1 xl:grid-cols-1 my-12  ">
                    ...
                    {
                        agyms.map(agym => <Gymdetils key={agym._id} agym={agym}></Gymdetils>)
