...
import Pvcdetils from './Pvcdetils';
...
const Pvc = () => {
    const [apvcs, setApvcs] = useState([]);
    useEffect(() => {
        fetch('https://test.zrsflooring.com/pvc')
            .then(res => res.json())
            .then(data => setApvcs(data));

    }, [])
    return (
        <div>
            <div className="-mt-24">
                <div className="h-[60vh] mt-24 bg-cover " style={{ backgroundImage: 'url(https://i.ibb.co/3N5MDZt/PVC-Flooring.jpg)', }}>
                    <div className="hero-overlay "></div>
                    <div className="  text-white -mt-56 ml-12 ">
                        <h1 className="text-3xl font-bold" >PVC Flooring</h1>
                        <p>We Are The PVC Flooring suppliers in UAE</p>
                    </div>
                </div>
            ...
            <div className=" xl:w-80 xl:h-[800px] lg:w-64 lg:h-[520px] md:w-64 md:h-[520px] w-20 h-[520px] bg-white shadow-xl border border-black text-black  rounded-lg bg-fixed ">
                ...
                <div className="w-[95%] xl:w-[90%] lg:w-[90%] mx-auto xl:gap-8 lg:gap-6 md:gap-10 gap-2 grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1 xl:grid-cols-1 my-12  ">
                    ...
                    {
                        apvcs.map(apvc => <Pvcdetils key={apvc._id} apvc={apvc}></Pvcdetils>)
